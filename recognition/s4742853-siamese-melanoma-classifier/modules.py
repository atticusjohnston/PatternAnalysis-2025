...
class PretrainedSiameseNetwork(nn.Module):
    ...
    def forward(self, x1, x2):
        if torch.rand(1) < 0.1:
            logger.info(f"x1: min={x1.min():.3f}, max={x1.max():.3f}, mean={x1.mean():.3f}")
            logger.info(f"x2: min={x2.min():.3f}, max={x2.max():.3f}, mean={x2.mean():.3f}")
            logger.info(f"x1==x2: {torch.allclose(x1, x2)}")

        h1 = self.forward_one(x1)
        h2 = self.forward_one(x2)

        if torch.rand(1) < 0.1:  # Print occasionally
